...
def main():
    ...
    if st.button('登録') == True:
        ...
        url = 'http://aoi.naist.jp/wellbeing_test'
        requests.post(url + '/post_labtest',json=data_post)
        st.write('入力完了しました！')

        r_diary = requests.get(url + '/get_diary_labtest', params={'user':name})
...
# ユーザ情報。引数
names = ['ito','aramaki','wakamiya','manabe','hisada','iida','ota'] 
usernames = ['ito','aramaki','wakamiya','manabe','hisada','iida','ota']   # 入力フォームに入力された値と合致するか確認される
passwords = ['ito','aramaki','wakamiya','manabe','hisada','iida','ota']   # 入力フォームに入力された値と合致するか確認される
